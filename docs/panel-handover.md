...
- `reactInspector/pathRequestCompletion.ts`: `reactInspectPath` 브리지 콜백 응답을 판별 유니온 completion(`success`/`failure`)으로 정규화해 controller onDone 분기를 단순화하는 규칙 전담
