...
1. 도메인에 맞는 모듈(`pageAgent.ts`, `pageAgentDom.ts`, `pageAgentInspect.ts`, `pageAgentFiberSearch.ts`, `pageAgentSerialization.ts`, `pageAgentCollectionPath.ts`, `pageAgentSerializerSummary.ts`, `pageAgentHookState.ts`, `pageAgentHookMetadata.ts`)에 구현 함수 추가
...
1. 도메인 성격에 맞는 모듈(`pageAgent.ts`, `pageAgentDom.ts`, `pageAgentInspect.ts`, `pageAgentFiberSearch.ts`, `pageAgentSerialization.ts`, `pageAgentCollectionPath.ts`, `pageAgentSerializerSummary.ts`, `pageAgentHookState.ts`, `pageAgentHookMetadata.ts`)에 함수 작성
