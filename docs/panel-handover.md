...
- `controller.ts`: panel DOM ref/이벤트 오케스트레이션을 유지하고, React Inspector 상태는 `reactInspector/controllerState.ts`를 통해 접근/갱신
