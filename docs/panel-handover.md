...
- 내부 순서:
  ...
  - `controller.ts`의 reactInspectPath 호출(`inspectFunctionAtPath`, `fetchSerializedValueAtPath`)은 공통 helper(`requestReactInspectPath`)로 요청/응답 검증 규칙을 재사용하고 completion 객체 기반 `onDone` 콜백으로 후처리 분기를 단순화
