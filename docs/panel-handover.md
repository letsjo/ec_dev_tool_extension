...
- 파일: `src/content/pageAgent.ts`, `src/content/pageAgentDom.ts`, `src/content/pageAgentBridge.ts`, `src/content/pageAgentMethods.ts`, `src/content/pageAgentHookGroups.ts`, `src/content/pageAgentHookStack.ts`, `src/content/pageAgentHookState.ts`, `src/content/pageAgentHookMetadata.ts`, `src/content/pageAgentInspect.ts`, `src/content/pageAgentFiberSearch.ts`, `src/content/pageAgentSerialization.ts`, `src/content/reactRuntimeHook.ts`
...
custom hook stack 파싱 유틸은 `src/content/pageAgentHookStack.ts`로, group path 추론 오케스트레이션은 `src/content/pageAgentHookGroups.ts`로 위임합니다.
...
- `pageAgentHookGroups.ts`: stack 파싱 유틸을 조합해 primitive hook 이름 정규화, custom hook group/path 추론 전담
