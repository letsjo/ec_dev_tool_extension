...
- `reactInspector/fetchOptions.ts`: `fetchReactInfo` 전달 옵션에서 applyOptions 조립, selectedComponentId 계산, runtime refresh/element selection 프리셋 전담
- `reactInspector/lookup.ts`: `lastReactLookup` 저장 갱신(keepLookup 규칙)과 runtime refresh 기본 lookup 계산 전담
