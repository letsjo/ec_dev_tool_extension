...
- 내부 순서:
  ...
  - `debugLog/debugLogFlow.ts`, `debugLog/debugDiagnosticsFlow.ts` 유틸로 패널 액션 로그 누적/복사/초기화 + dev-only diagnostics 이벤트/오류 집계(`ecDevTool.devDiagnostics=1`) 위임
...
- `panel.html`
  ...
  - Selected Element / DOM Path / Selected DOM Tree / Raw Result / Debug Log(+dev diagnostics)의 스크롤/배경/최소높이 제어
