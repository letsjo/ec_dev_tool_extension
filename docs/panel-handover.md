...
- 내부 순서:
  ...
  - `reactInspector/detailFetchQueue.ts`, `reactInspector/detailFetchQueueState.ts` 유틸로 선택 컴포넌트 상세 지연조회 큐(in-flight/queue/cooldown)와 queue 상태 전이 helper 위임
...
- `reactInspector/detailFetchQueue.ts`: 선택 컴포넌트 상세 데이터 지연 조회 큐 오케스트레이션, 실패 문구/응답 판정 분기, queue state helper 결선 전담
