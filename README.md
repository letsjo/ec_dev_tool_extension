...
7. **Select element**를 누르면 hover/focus 대상에 맞춰 `Selected Element / DOM Path / Selected DOM Tree`가 실시간으로 미리보기 갱신된다. 클릭하거나 `Enter`를 누르면 현재 하이라이트 요소를 우선 선택하고, 하이라이트가 없을 때만 포커스된 요소를 선택 확정해 React 컴포넌트 체인을 조회한다. `Esc`는 선택 모드를 취소한다. 이 단축키는 페이지/패널 어느 쪽에 포커스가 있어도 picker 활성 상태라면 동작하도록 중계된다.
